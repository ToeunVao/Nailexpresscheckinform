...
// --- Fixed Section (uniqueNames and uniquePhones) ---
const openAddAppointmentModal = (date, clientData = null) => {
    ...
    // ✅ FIXED: replaced invalid syntax with Array.from(new Set(...))
    const uniqueNames = Array.from(new Set(allFinishedClients.map(c => c.name)));
    const uniquePhones = Array.from(new Set(
        allFinishedClients.filter(c => c.phone && c.phone !== 'N/A').map(c => c.phone)
    ));
...
function initMainApp(userRole) {
    ...
    const updateDashboard = () => {
        ...
        // This logic now correctly handles filtering for the dashboard chart AND the new table
        let dashboardEarningsData = allEarnings;
        if (userRole !== 'admin') {
            dashboardEarningsData = allEarnings.filter(e => e.staffName === currentUserName);
        }
        updateDashboardEarningsChart(dashboardEarningsData, filter);
        renderDashboardFullStaffEarnings(applyEarningFilters(dashboardEarningsData, currentDashboardEarningTechFilter, currentDashboardEarningDateFilter, currentDashboardEarningRangeFilter));
    };
